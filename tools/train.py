...
def train(model, dataloader, optimizer, scheduler, loss_fn, device, feat_len, batch_size, epoch, epochs, lr):
    ...
    pbar = tqdm(enumerate(dataloader), total=iters_per_epoch, desc=f"Epoch: [{epoch+1}/{epochs}] Iter: [{0}/{iters_per_epoch}] Loss: {loss_per_epoch:.8f}")
    ...
    for iter, (tokens, mask, img_feat, _, _) in pbar:
        ...
        pbar.set_description(f"Epoch: [{epoch+1}/{epochs}] Iter: [{iter+1}/{iters_per_epoch}] Loss: {loss_per_epoch / (iter+1):.8f}")
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % cfg['EVAL_INTERVAL'] == 0 or epoch == epochs-1:
            ...
            if val_loss < best_loss:
                best_loss = val_loss
            torch.save(model.state_dict(), save_dir / f"{cfg['MODEL']}_{cfg['DATASET']}_{cfg['BATCH_SIZE']}_{cfg['SEQ_LENGTH']}.pth")
